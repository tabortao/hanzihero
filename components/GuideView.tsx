...
import { ArrowLeft, BookOpen, Layout, Gamepad2, FileText, BarChart3, Settings, Rocket, Target, Book } from 'lucide-react';
       ...
       <div className="p-6 md:p-10 space-y-12">
          ...
          <section>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 <FeatureItem icon={<Target className="text-blue-500"/>} title="识字 (Home)" desc="同步人教版等主流教材。提供笔顺动画、田字格、拼音及AI辅助讲解。" bg="bg-blue-50" />
                 <FeatureItem icon={<Gamepad2 className="text-purple-500"/>} title="游戏化学习" desc="多种趣味模式：智趣翻卡屋、星箭追击、超能靶场、汉字消消乐。" bg="bg-purple-50" />
                 <FeatureItem icon={<FileText className="text-amber-500"/>} title="阅读 (Story)" desc="独创 AI 分级阅读。系统根据孩子已掌握的汉字库，自动生成适合其阅读水平的趣味短文。" bg="bg-amber-50" />
                 <FeatureItem icon={<BarChart3 className="text-pink-500"/>} title="统计 (Stats)" desc="基于艾宾浩斯曲线的记忆管理。直观展示学习热力图、遗忘曲线分布。" bg="bg-pink-50" />
                 <FeatureItem icon={<Settings className="text-gray-500"/>} title="个人中心 (Profile)" desc="支持配置 AI 模型、调整每日学习量、语速设置，以及数据备份。" bg="bg-gray-50" />
             </div>
          </section>
...
const FeatureItem = ({icon, title, desc, bg}: any) => (
    <div className={`flex gap-4 p-5 rounded-2xl border border-transparent hover:border-gray-200 hover:shadow-sm transition-all ${bg}`}>
        <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center shrink-0 shadow-sm">{icon}</div>
        <div>
...
const StepItem = ({step, title, content}: any) => (
    <div className="flex gap-4">
        <div className="flex flex-col items-center">
            <span className="flex items-center justify-center w-8 h-8 bg-green-500 rounded-full text-white font-bold shadow-sm z-10">{step}</span>
